...
;保存中断允许寄存器~PRIMASK 这个是只有一位的寄存器,参考M3权威指南--白菜注~
...
;PENDSV中断,M3的所谓可悬挂请求中断,类似于当年ARM7的软中断吧~主要又上面的OSCtxSw,OSIntCtxSw触发,也就是
;			可人为触发的中断.名字必须符合库函数里面PENDSV的声明名字
;			关于PENDSV中断的用处,参照CM3权威指南~PENDSV是专为OS而弄出来的一个中断(可悬挂)--白菜注
...
PendSV_Handler													;此名称要与startup_stm32f10x_md.s（启动代码）中的PendSV的中断向量名保持一致
